...
x, y, intensity = np.loadtxt(fname, delimiter=',', unpack=True)
...
i_vals = np.array(intensity, dtype=float).reshape((x_vals.size, y_vals.size))
...
if plottype == 'grid':
    plt.imshow(np.rot90(i_vals), extent=[x_vals[0], x_vals[-1], y_vals[0], y_vals[-1]], aspect='auto')
