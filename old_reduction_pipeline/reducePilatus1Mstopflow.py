...
# modified 11/12/2014 for Pilatus1M stopflow experiment Geeves
def convertLog2Header(logfile, headerfile):
  print logfile
  os.chdir(headerPath)
  ...
  with open(clonedLogFile, 'r') as f:
    ...
    line = f.readline() # read the second line data
    columns = line.split()
      #print columns
      #print len(columns)
    try:
...
proc.stdin.write('set xrange [0.004:0.5]\n')
...
for tif in newlist:
  ...
  if not os.path.isfile(os.path.join(datPath, tifname[:-3] + 'dat')):
    headerFile = os.path.join(headerPath,  tifname[:-3] + 'txt')
    if not os.path.isfile(headerFile):
      convertLog2Header(tifname[:-9] + '.log', headerFile)
