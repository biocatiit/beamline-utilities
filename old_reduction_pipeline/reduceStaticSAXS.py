  ...
  with open(clonedLogFile, 'r') as f:
    ...
    for line in f:
      ...
      try:
        ...
        #---Normalized against I0---Weifeng Shang-20150319-
        #fout.writelines('Transmitted Beam: %s\n' %transmitted)
        fout.writelines('Transmitted Beam: %s\n' %incident)
        #----------------------------Weifeng Shang-20150319-
...
while True:
  newlist = sorted(glob.glob(tifPath + '????_' + fileRootName + '_*.tif'))
  #print newlist
  for tif in newlist:
    path, tifname = os.path.split(tif)
    
    #if not os.path.isfile(os.path.join(datPath, tifname[:-3] + 'dat')):
    if not os.path.isfile(os.path.join(datPath, 'dat')):
