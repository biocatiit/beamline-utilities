...
print 'python scanEnergyMono1.py start_E end_E step'
...
def getDark():
  status = epics.caput(SHUTTER, shutter_close, wait=True, timeout=60)
  time.sleep(0.05) # Wait 50 ms for slow shutter to close
 
  # start scaler 
  status = epics.caput(SCALER_SET_TIME, EXPOSURE_TIME, wait=True, timeout=60)
  status = epics.caput(SCALER_START, 1, wait=True, timeout=60)
  #time.sleep(EXPOSURE_TIME+0.2)   # Wait for scaler to complete
  # read scaler values
